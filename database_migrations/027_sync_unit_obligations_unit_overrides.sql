-- Тарифа за период: COALESCE(индивидуална по единица, сума по група за период, 0).
-- Изисква: 026 (period_unit_amounts), 019.
...
BEGIN
  ...
  LOOP
    ...
    SELECT COALESCE(
      (
        SELECT pua.amount
        FROM public.period_unit_amounts pua
        WHERE pua.period_id = p_period_id AND pua.unit_id = r_unit.id
      ),
      (
        SELECT pga.amount
...
COMMENT ON FUNCTION public.sync_unit_obligations_for_period(UUID) IS
  'Обновява редовете unit_obligations за период: сума = period_unit_amounts за единица, иначе period_group_amounts по група; запазва погасената част; един тарифен ред по (единица, период).';
